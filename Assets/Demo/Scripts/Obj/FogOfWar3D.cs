...
public class FogOfWar3D : MonoBehaviour
{
	...
	public Vector3 m_viewerCenter = Vector3.zero; //世界坐标
    ...
    float m_areaAspect = 1.0f; //区域的横纵比
    void Start()
    {
        CreateMaterial();
	...
	void OnPreRender()
	{
		...
		{
			Vector3 viewerCenter = m_viewerCenter;
			if (m_viewer != null)
			{
				viewerCenter = m_viewer.position;
			}
			Vector4 viewCenter = TransformWorldCoordToUVCoord(viewerCenter);
			viewCenter.z = m_brushSize;
			viewCenter.w = m_edgeSmoothValue;
			m_fogOfWarMaterial.SetVector("_ViewCenter", viewCenter);
			//debug
			m_fogOfWarMaterial.SetTexture("_NoiseTex", m_perlinNoise);
            m_fogOfWarMaterial.SetFloat("_BufferAspect", m_areaAspect);
			m_fogOfWarMaterial.SetFloat("_FogDensity", m_fogDensity);
    ...
    void CreateMaterial()
    {
        m_fogOfWarMaterial = new Material(Shader.Find("FogOfWar/FogOfWar"));
        m_fogOfWarCastMaterial = new Material(Shader.Find("FogOfWar/FogOfWarCast"));
    }
