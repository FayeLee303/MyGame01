using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class WeaponObj3D : MonoBehaviour {

    public WeaponModel weapon { get; private set; }//外部只读，只能在这个类内部更改

    private Sprite currentSprite;
    private BoxCollider boxCollider;//实际的碰撞盒

    private Sprite Sprite
    {
        get { return GetComponentInChildren<SpriteRenderer>().sprite; }
        set { GetComponentInChildren<SpriteRenderer>().sprite = value; }
    }
    private BoxCollider BoxCollider
    {
        get { return GetComponent<BoxCollider>(); }
    }

    //设置Item
    public void SetWeapon(WeaponModel weapon)
    {
        this.weapon = weapon;
        SetSprite(weapon);
        SetColliderBox();//重新设置碰撞盒子
    }
    //设置图片
    private void SetSprite(WeaponModel weapon)
    {
        Sprite = Resources.Load<Sprite>(weapon.SpritePath);
    }
    //设置碰撞盒
    private void SetColliderBox()
    {
        float width = transform.localScale.x * Sprite.bounds.size.x;
        float hight = transform.localScale.y * Sprite.bounds.size.y;
        BoxCollider.size = new Vector3(width, (float)(hight / 1.414), (float)(hight / 1.414));
        SetLocalPositionY((float)(hight / 1.414 / 2));//重设y坐标
    }
    //调整y坐标
    private void SetLocalPositionY(float y)
    {
        transform.position = new Vector3(transform.position.x, y, transform.position.z);
    }

    //碰撞拾取
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag == "Player")
        {
            if (WeaponPanel.Instance.FindSameTypeSlot(this.weapon) == true) //如果碰到的东西已经在道具槽里有一个格子装着一样的东西
            {
                InventoryManager.Instance.ShowInfoBox("想装备的武器类型重复");
            }
            else//格子都没有装同样的东西
            {
                WeaponSlot _slot = WeaponPanel.Instance.FindEmptyWeaponSlot();
                if (_slot != null) //如果有空格子就存进去
                {
                    _slot.StoreWeapon(this.weapon);
                    InventoryManager.Instance.ShowInfoBox("拾取了1个" + this.weapon.Name);
                    Destroy(gameObject);
                }
                else//没有空格子就提示背包已满
                {
                    InventoryManager.Instance.ShowInfoBox("武器格子已经满了");
                }
            }
        }
    }
}
